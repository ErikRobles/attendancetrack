(function () {
    const el1 = document.querySelector("#el1");
    el1.style.display = "none";
    const el2 = document.querySelector("#el2");
    el2.style.display = "none";
    const el3 = document.querySelector("#el3");
    el3.style.display = "none";
    const el4 = document.querySelector("#el4");
    el4.style.display = "none";
    const el5 = document.querySelector("#el5");
    el5.style.display = "none";
    const el6 = document.querySelector("#el6");
    el6.style.display = "none";
    const el7 = document.querySelector("#el7");
    el7.style.display = "none";
    const el8 = document.querySelector("#el8");
    el8.style.display = "none";
    const el9 = document.querySelector("#el9");
    el9.style.display = "none";
    const el10 = document.querySelector("#el10");
    el10.style.display = "none";
    const el11 = document.querySelector("#el11");
    el11.style.display = "none";
    const el12 = document.querySelector("#el12");
    el12.style.display = "none";
    const el13 = document.querySelector("#el13");
    el13.style.display = "none";
    const el14 = document.querySelector("#el14");
    el14.style.display = "none";
    const el15 = document.querySelector("#el15");
    el15.style.display = "none";
    const pr1 = document.querySelector("#pr1");
    pr1.style.display = "none";
    const pr2 = document.querySelector("#pr2");
    pr2.style.display = "none";
    const pr3 = document.querySelector("#pr3");
    pr3.style.display = "none";
    const pr4 = document.querySelector("#pr4");
    pr4.style.display = "none";
    const pr5 = document.querySelector("#pr5");
    pr5.style.display = "none";
    const pr6 = document.querySelector("#pr6");
    pr6.style.display = "none";
    const pr7 = document.querySelector("#pr7");
    pr7.style.display = "none";
    const pr8 = document.querySelector("#pr8");
    pr8.style.display = "none";
    const pr9 = document.querySelector("#pr9");
    pr9.style.display = "none";
    const pr10 = document.querySelector("#pr10");
    pr10.style.display = "none";
    const pr11 = document.querySelector("#pr11");
    pr11.style.display = "none";
    const pr12 = document.querySelector("#pr12");
    pr12.style.display = "none";
    const pr13 = document.querySelector("#pr13");
    pr13.style.display = "none";
    const pr14 = document.querySelector("#pr14");
    pr14.style.display = "none";
    const pr15 = document.querySelector("#pr15");
    pr15.style.display = "none";
    const in1 = document.querySelector("#in1");
    in1.style.display = "none";
    const in2 = document.querySelector("#in2");
    in2.style.display = "none";
    const in3 = document.querySelector("#in3");
    in3.style.display = "none";
    const in4 = document.querySelector("#in4");
    in4.style.display = "none";
    const in5 = document.querySelector("#in5");
    in5.style.display = "none";
    const in6 = document.querySelector("#in6");
    in6.style.display = "none";
    const in7 = document.querySelector("#in7");
    in7.style.display = "none";
    const in8 = document.querySelector("#in8");
    in8.style.display = "none";
    const in9 = document.querySelector("#in9");
    in9.style.display = "none";
    const in10 = document.querySelector("#in10");
    in10.style.display = "none";
    const in11 = document.querySelector("#in11");
    in11.style.display = "none";
    const in12 = document.querySelector("#in12");
    in12.style.display = "none";
    const in13 = document.querySelector("#in13");
    in13.style.display = "none";
    const in14 = document.querySelector("#in14");
    in14.style.display = "none";
    const in15 = document.querySelector("#in15");
    in15.style.display = "none";
    const up1 = document.querySelector("#up1");
    up1.style.display = "none";
    const up2 = document.querySelector("#up2");
    up2.style.display = "none";
    const up3 = document.querySelector("#up3");
    up3.style.display = "none";
    const up4 = document.querySelector("#up4");
    up4.style.display = "none";
    const up5 = document.querySelector("#up5");
    up5.style.display = "none";
    const up6 = document.querySelector("#up6");
    up6.style.display = "none";
    const up7 = document.querySelector("#up7");
    up7.style.display = "none";
    const up8 = document.querySelector("#up8");
    up8.style.display = "none";
    const up9 = document.querySelector("#up9");
    up9.style.display = "none";
    const up10 = document.querySelector("#up10");
    up10.style.display = "none";
    const up11 = document.querySelector("#up11");
    up11.style.display = "none";
    const up12 = document.querySelector("#up12");
    up12.style.display = "none";
    const up13 = document.querySelector("#up13");
    up13.style.display = "none";
    const up14 = document.querySelector("#up14");
    up14.style.display = "none";
    const up15 = document.querySelector("#up15");
    up15.style.display = "none";
    const ad1 = document.querySelector("#ad1");
    ad1.style.display = "none";
    const ad2 = document.querySelector("#ad2");
    ad2.style.display = "none";
    const ad3 = document.querySelector("#ad3");
    ad3.style.display = "none";
    const ad4 = document.querySelector("#ad4");
    ad4.style.display = "none";
    const ad5 = document.querySelector("#ad5");
    ad5.style.display = "none";
    const ad6 = document.querySelector("#ad6");
    ad6.style.display = "none";
    const ad7 = document.querySelector("#ad7");
    ad7.style.display = "none";
    const ad8 = document.querySelector("#ad8");
    ad8.style.display = "none";
    const ad9 = document.querySelector("#ad9");
    ad9.style.display = "none";
    const ad10 = document.querySelector("#ad10");
    ad10.style.display = "none";
    const ad11 = document.querySelector("#ad11");
    ad11.style.display = "none";
    const ad12 = document.querySelector("#ad12");
    ad12.style.display = "none";

    const luc = document.querySelector("#lastUnitCovered");
    window.onload = function () {
        luc.focus();
    };
    luc.addEventListener("focus", (e) => {
        const v = e.target.value;
        el1.style.display = "none";
        el2.style.display = "none";
        el3.style.display = "none";
        el4.style.display = "none";
        el5.style.display = "none";
        el6.style.display = "none";
        el7.style.display = "none";
        el8.style.display = "none";
        el9.style.display = "none";
        el10.style.display = "none";
        el11.style.display = "none";
        el12.style.display = "none";
        el13.style.display = "none";
        el14.style.display = "none";
        el15.style.display = "none";
        pr1.style.display = "none";
        pr2.style.display = "none";
        pr3.style.display = "none";
        pr4.style.display = "none";
        pr5.style.display = "none";
        pr6.style.display = "none";
        pr7.style.display = "none";
        pr8.style.display = "none";
        pr9.style.display = "none";
        pr10.style.display = "none";
        pr11.style.display = "none";
        pr12.style.display = "none";
        pr13.style.display = "none";
        pr14.style.display = "none";
        pr15.style.display = "none";
        in1.style.display = "none";
        in2.style.display = "none";
        in3.style.display = "none";
        in4.style.display = "none";
        in5.style.display = "none";
        in6.style.display = "none";
        in7.style.display = "none";
        in8.style.display = "none";
        in9.style.display = "none";
        in10.style.display = "none";
        in11.style.display = "none";
        in12.style.display = "none";
        in13.style.display = "none";
        in14.style.display = "none";
        in15.style.display = "none";
        up1.style.display = "none";
        up2.style.display = "none";
        up3.style.display = "none";
        up4.style.display = "none";
        up5.style.display = "none";
        up6.style.display = "none";
        up7.style.display = "none";
        up8.style.display = "none";
        up9.style.display = "none";
        up10.style.display = "none";
        up11.style.display = "none";
        up12.style.display = "none";
        up13.style.display = "none";
        up14.style.display = "none";
        up15.style.display = "none";
        ad1.style.display = "none";
        ad2.style.display = "none";
        ad3.style.display = "none";
        ad4.style.display = "none";
        ad5.style.display = "none";
        ad6.style.display = "none";
        ad7.style.display = "none";
        ad8.style.display = "none";
        ad9.style.display = "none";
        ad10.style.display = "none";
        ad11.style.display = "none";
        ad12.style.display = "none";

        if (v === "el1") {
            el1.style.display = "block";
        } else if (v === "el2") {
            el2.style.display = "block";
        } else if (v === "el3") {
            el3.style.display = "block";
        } else if (v === "el4") {
            el4.style.display = "block";
        } else if (v === "el5") {
            el5.style.display = "block";
        } else if (v === "el6") {
            el6.style.display = "block";
        } else if (v === "el7") {
            el7.style.display = "block";
        } else if (v === "el8") {
            el8.style.display = "block";
        } else if (v === "el9") {
            el9.style.display = "block";
        } else if (v === "el10") {
            el10.style.display = "block";
        } else if (v === "el11") {
            el11.style.display = "block";
        } else if (v === "el12") {
            el12.style.display = "block";
        } else if (v === "el13") {
            el13.style.display = "block";
        } else if (v === "el14") {
            el14.style.display = "block";
        } else if (v === "el15") {
            el15.style.display = "block";
        } else if (v === "pr1") {
            pr1.style.display = "block";
        } else if (v === "pr2") {
            pr2.style.display = "block";
        } else if (v === "pr3") {
            pr3.style.display = "block";
        } else if (v === "pr4") {
            pr4.style.display = "block";
        } else if (v === "pr5") {
            pr5.style.display = "block";
        } else if (v === "pr6") {
            pr6.style.display = "block";
        } else if (v === "pr7") {
            pr7.style.display = "block";
        } else if (v === "pr8") {
            pr8.style.display = "block";
        } else if (v === "pr9") {
            pr9.style.display = "block";
        } else if (v === "pr10") {
            pr10.style.display = "block";
        } else if (v === "pr11") {
            pr11.style.display = "block";
        } else if (v === "pr12") {
            pr12.style.display = "block";
        } else if (v === "pr13") {
            pr13.style.display = "block";
        } else if (v === "pr14") {
            pr14.style.display = "block";
        } else if (v === "pr15") {
            pr15.style.display = "block";
        } else if (v === "in1") {
            in1.style.display = "block";
        } else if (v === "in2") {
            in2.style.display = "block";
        } else if (v === "in3") {
            in3.style.display = "block";
        } else if (v === "in4") {
            in4.style.display = "block";
        } else if (v === "in5") {
            in5.style.display = "block";
        } else if (v === "in6") {
            in6.style.display = "block";
        } else if (v === "in7") {
            in7.style.display = "block";
        } else if (v === "in8") {
            in8.style.display = "block";
        } else if (v === "in9") {
            in9.style.display = "block";
        } else if (v === "in10") {
            in10.style.display = "block";
        } else if (v === "in11") {
            in11.style.display = "block";
        } else if (v === "in12") {
            in12.style.display = "block";
        } else if (v === "in13") {
            in13.style.display = "block";
        } else if (v === "in14") {
            in14.style.display = "block";
        } else if (v === "in15") {
            in15.style.display = "block";
        } else if (v === "up1") {
            up1.style.display = "block";
        } else if (v === "up2") {
            up2.style.display = "block";
        } else if (v === "up3") {
            up3.style.display = "block";
        } else if (v === "up4") {
            up4.style.display = "block";
        } else if (v === "up5") {
            up5.style.display = "block";
        } else if (v === "up6") {
            up6.style.display = "block";
        } else if (v === "up7") {
            up7.style.display = "block";
        } else if (v === "up8") {
            up8.style.display = "block";
        } else if (v === "up9") {
            up9.style.display = "block";
        } else if (v === "up10") {
            up10.style.display = "block";
        } else if (v === "up11") {
            up11.style.display = "block";
        } else if (v === "up12") {
            up12.style.display = "block";
        } else if (v === "up13") {
            up13.style.display = "block";
        } else if (v === "up14") {
            up14.style.display = "block";
        } else if (v === "up15") {
            up15.style.display = "block";
        } else if (v === "ad1") {
            ad1.style.display = "block";
        } else if (v === "ad2") {
            ad2.style.display = "block";
        } else if (v === "ad3") {
            ad3.style.display = "block";
        } else if (v === "ad4") {
            ad4.style.display = "block";
        } else if (v === "ad5") {
            ad5.style.display = "block";
        } else if (v === "ad6") {
            ad6.style.display = "block";
        } else if (v === "ad7") {
            ad7.style.display = "block";
        } else if (v === "ad8") {
            ad8.style.display = "block";
        } else if (v === "ad9") {
            ad9.style.display = "block";
        } else if (v === "ad10") {
            ad10.style.display = "block";
        } else if (v === "ad11") {
            ad11.style.display = "block";
        } else if (v === "ad12") {
            ad12.style.display = "block";
        }
    });
})();
